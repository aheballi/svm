...
$ConfigParent="\\10.80.151.230\Hyper-VAutomation\RevertVMsInstallLatestASBuild"
$SharedBuildLoc = "\\10.80.151.230\Hyper-VAutomation\LastLatestASBuild"
...
$FullBuildPath =  $BuildLocation +"\" +$global:GreatestBuildNo + "\"+ $BuildSubPath+"\"+$InstallerExeName
  ...
  if(($Global:GreatestBuildNo -ne "0"))
    {
      Write-Message ("Removing the earlier build # $LastBuildNo  From  " + $SharedBuildLoc ) $DetLogFile
      Get-ChildItem -Path $SharedBuildLoc -Recurse| Foreach-object {Remove-item -Recurse -path $_.FullName -Force }
      $BuildNoFolder=New-Item -Path $SharedBuildLoc -Name $global:GreatestBuildNo -ItemType "Directory"
      ...
      Copy-Item $FullBuildPath -Destination $SharedBuildLocwithBuildNo
      $ExePath=$SharedBuildLocwithBuildNo+"\"+$InstallerExeName
            ...
            $VMBuildFolder="\\"+$VMHostName+"\Build"
              if($ToRevert -eq 1)
               ...
               if($ToInstallLatestBuild -eq 1)
               {
                ...
                $InstallerEXE = $SharedBuildLocwithBuildNo+"\"+$InstallerExeName
                ...
                Invoke-Command -Session $TargetVMSession -ScriptBlock {Param($SharedBuildLoc, $InstallerEXE, $VMBuildFolder, $LogFile, $Global:ScriptName, $global:GreatestBuildNo,$AdminStudioVersion, $TargetVMName, $InstallerExeName, $DetLogFile)
                                                              ...
                                                              $ThrowOutResult = net use $SharedBuildLocwithBuildNo $CommandLineUser1 $password1
                                                         ...
                                                         }-ArgumentList $SharedBuildLocwithBuildNo,$InstallerEXE,$VMBuildFolder,$LogFile,$Global:ScriptName,$global:GreatestBuildNo,$AdminStudioVersion, $TargetVMName, $InstallerExeName, $DetLogFile
