...
#start
$TestName='WrappedEXEPublish_ToSCCM_And_Validate'

$ParentFolder = "C:\AS_Automation"
$TestCaseName="WrappedEXEPublish_ToSCCM_And_Validate"
...
UpdateDependencies $Dep2SqlPath $wrapid $depPkgID1 $depPkgID2 $CatalogName
...
{
    Write-host (Write-Header $TestName) "Publish wrapped EXE to SCCM Test Case Failed"
    $retval=-1
}
Else 
{
    Write-host (Write-Header $TestName) "Publish wrapped EXE to SCCM Test Case Passed"
    $retval=0
}
}
else
...
#DeleteCatalog WrappedEXEPublish_ToSCCM_And_Validate