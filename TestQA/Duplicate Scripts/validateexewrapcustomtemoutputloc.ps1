...
$TestCaseName="validateexewrapcustomtemoutputloc"

$strSourcePath= "\\10.20.150.10\AdminStudio\AS_Automation\TestCases\"+$TestCaseName
$CatalogName ="validateexewrapcustomtemoutputloc"
...
if($RetVal -eq 0)
{
	...
	Write-Host (Write-Header $TestCaseName) "Validation of $Package wrap output files is passed"
	...
	File Name    : $CustomOutputFolder\vlc-2.2.4-win32_$PkgID\Files\$exePackage
	Product Code :
	Version      : 0.0.0"
...
Write-Host (Write-Header $TestCaseName) "Validation of $Package wrap output files is failed"
}
