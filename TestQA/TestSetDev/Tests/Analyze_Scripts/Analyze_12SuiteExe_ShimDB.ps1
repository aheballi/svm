...
$folderExe          = "\\10.80.150.184\Automation_TestData\Analyze\SuiteExe_ShimDB\Packages\SuiteExePackages"
If ( (Get-WmiObject Win32_OperatingSystem).OSArchitecture -eq "64-bit" )
{
    $TestResultFile     = "\\10.80.150.184\Automation_TestData\Analyze\SuiteExe_ShimDB\Packages\TestResults_64.txt"
    $DLLFolder = "C:\Program Files (x86)\AdminStudio\2019\Common\Tools"
}
else
{
    $TestResultFile     = "\\10.80.150.184\Automation_TestData\Analyze\SuiteExe_ShimDB\Packages\TestResults_32.txt"
    $DLLFolder = "C:\Program Files\AdminStudio\2019\Common\Tools"
}

#$TestResultFile     = "C:\TestSetDev\Tests\TestResults.txt"
$global:CatalogName = 'NewCatalog_SuiteExeTest'
$global:TestErrorCount = 0

#CopyInnoSetupDll
   $dllInnosetup = "innounp.exe"
   $strDLLSourcePath = "\\10.80.150.184\Automation_TestData\Analyze\SuiteExe_ShimDB\SuiteEXEDLLS\InnoSetup\" + $dllInnosetup
   $DllFile = $DLLFolder + "\" + $dllInnosetup

   <#CopyInstallAnywhereDll
   $dllInstallAnywhere = "7z920x86.dll"
   $strDLLSourcePathInstallAnywhere = "\\10.80.150.184\Automation_TestData\Analyze\SuiteExe_ShimDB\SuiteEXEDLLS\InstallAnyWhere\" + $dllInstallAnywhere
   $DLLFolderInstallAnywhere = "C:\Program Files (x86)\AdminStudio\2016\Common"
   $DllFileInstallAnywhere = "C:\Program Files (x86)\AdminStudio\2016\Common\" + $dllInstallAnywhere#>
...
{
    $Header = ('['+(Get-Date -Format 'hh:mm:ss')+']') + ' [004SuiteExe_ShimDB]'
    return $Header
}
...
{
    $global:oPkgArray   = @()
    #$global:CatalogName = 'NewCatalog_SuiteExeTest_01' + $Name
    #$result = New-ASCatalog -CatalogName $global:CatalogName
    $result = New-ASCatalog -CatalogName $Name
}
#>
...
{ 
    if ($Package.RowId -ne 0)
    {
        ...
        #Write-Host ($global:nPackageCount)
    }
}
...
#remove-item -path $DllFileInstallAnywhere -force 
Get-Process AdminStudioHost | Stop-Process
exit 0
}
catch
