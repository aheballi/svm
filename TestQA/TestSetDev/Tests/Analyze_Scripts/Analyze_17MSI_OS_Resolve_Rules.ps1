...
$folderMSI          = "\\10.80.150.184\Automation_TestData\Analyze\MSI_OS_Resolve\Packages"
If ( (Get-WmiObject Win32_OperatingSystem).OSArchitecture -eq "64-bit" )
{
    $TestResultFile     = "\\10.80.150.184\Automation_TestData\Analyze\MSI_OS_Resolve\TestResults_64.txt"
    #$DLLFolder = "C:\Program Files (x86)\AdminStudio\2019\Common\Tools"
}
else
{
    $TestResultFile     = "\\10.80.150.184\Automation_TestData\Analyze\MSI_OS_Resolve\TestResults_32.txt"
    #$DLLFolder = "C:\Program Files\AdminStudio\2019\Common\Tools"
}

#$TestResultFile     = "C:\TestSetDev\Tests\TestResults.txt"
$global:CatalogName = 'MSI_OS_Resolve'
...
{
    $Header = ('['+(Get-Date -Format 'hh:mm:ss')+']') + ' [MSI_Rules]'
    return $Header
}
...
function Test ($o)
{
   ...
   if ($Total -eq 0)
   {
        Write-Host (Write-Header) 'Failed Found 0 errors in package for first import (There should be at least one):' $o.DisplayedProductName -foregroundcolor red
        #$global:TestErrorCount = $global:TestErrorCount + 1
		Write-Host ''
   }
...
function ReTestAfterResolve($Reimport){
    ...
    foreach ($RTestResult in $RTest.TestResults) 
    {
       $strPackageloc= $_.Package_Location
        Write-Host (Write-Header) "location: " $strPackageloc
   ...
   Write-Host (Write-Header) 'Total errors/warnings after resolving of issues:'  $ResolveTotalFailures -foregroundcolor yellow
