...
$TestName = "018IPA_OS_BP_RA_Rules"

Write-Host (Write-Header $TestName) "Test Started"

$ParentFolder = "C:\AS_Automation"
#$arr=@("OS_Rules","BP_Rules","RA_Rules")
$arr=@("OS_Rules","RA_Rules")
$SRFlag=0
$CatalogName= "IPA_OS_BP_RA_Rules"
$TestRunStatus=0

$TestCaseName="IPA_OS_BP_RA_Rules"
...
{ 
    $IPARules=$arr[$i]
    $strSourcePath = "\\10.80.150.184\Automation_TestData\Analyze\IPA_OS_BP_RA_Rules\CSVs\"+ $arr[$i]+".csv"
    $csvFileLocation=$CSVFolder+"\"+$arr[$i]+".csv"
     
    $Retval=CopyTestDataLocally $strSourcePath $CSVFolder 0
    WriteResultsToFile $logFile "Copy of $IPARules CSV test data is-" $Retval 0 
    ...
    {       
        $TestRunStatus =-1
        WriteResultsToFile $logFile "IPA $IPARules Test case " -1 0 
        Write-Host (Write-Header $TestName) "IPA $IPARules Test case Failed"       
    } 
    else
    {
        WriteResultsToFile $logFile "IPA $IPARules Test case " 0 0 
        Write-Host (Write-Header $TestName) "IPA $IPARules Test case Passed" 
    }  
...
IF($TestRunStatus -eq 0)
{
 ...
 Write-Host (Write-Header $TestName) "IPA Operating System,Best practices and Risk Assessment Rules validation Test cases Passed `r`n"
...
{
 Write-Host (Write-Header $TestName) "IPA operating System,Best practices and Risk Assessment Rules validation Test cases Failed `r`n"
 Get-Process AdminStudioHost | Stop-Process
 exit 1
}
