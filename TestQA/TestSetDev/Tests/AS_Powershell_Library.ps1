...
function DeleteCatalog ($CatalogName)
{
   Try
   {
    ...
    $connectionstring = "Data Source=$SQLServer;User ID=$UserName;user ID=sa;password=Flexera!;"    
...
Function CreateNewCatalog($CatalogName,$SRFlag)
{
    ...
    $setDB = "PROVIDER=SQLNCLI11;Data Source=$SQLServer;Initial Catalog=$CatalogName;user ID=sa;password=Flexera!;"
...
Function ConnectToCatalog($CatalogName)
{
   ...
   Set-ASConfigPlatform -ConnectionString "PROVIDER=SQLNCLI11;Data Source=$SQLServer;Initial Catalog=$CatalogName;user ID=sa;password=Flexera!;"    
   return 0
}
...
Function FolderImport($Packageslocation,$PackageTypes,$Group,$CatalogName)
{
    $retval=-1
    If(Test-Path $Packageslocation)
    {
        PrepAS

        #Set-ASConfigPlatform -ConnectionString "PROVIDER=SQLNCLI11;Data Source=$SQLServer;Initial Catalog=$CatalogName;Integrated Security=SSPI;"
        Set-ASConfigPlatform -ConnectionString "PROVIDER=SQLNCLI11;Data Source=$SQLServer;Initial Catalog=$CatalogName;user ID=sa;password=Flexera!;"
        $FolderImport = Invoke-ASImportFolder -FolderPath $Packageslocation -PackageTypes $PackageTypes -Group "Applications" -ApplySupportFiles 0 -CreateSubGroups 0
        
        if($FolderImport)
        {
            if($FolderImport.RowID -gt 0)
            {
                $retval= $FolderImport.RowID
                $packagename = $FolderImport.ProductName
            }
        }        
    }   
    return $packagename
...
Function ExecuteSQLQuery($strSQLQuery, $CatalogName)
{
    ...
    If($strSQLQuery)
    {
     ...
     $connectionString = "Data Source=$SQLServer;user ID=sa;password=Flexera!;Initial Catalog=$catalogName"
    ...
    $PropertyValue = Invoke-Command -Session $sessionID -ScriptBlock{
        ...
        Switch($PropertyName)
        {
            'Installation behavior'{
                $PropertyValue=$SDMPackageXML.DeploymentTypes[0].Installer.ExecutionContext
                }
    ...
    $PropertyValue = Invoke-Command -Session $sessionID -ScriptBlock{
        ...
        Switch($PropertyName)
        {
            'Administrator Comments'{
             ...
             'Installation behavior'{
                $PropertyValue=$SDMPackageXML.DeploymentTypes[0].Installer.ExecutionContext
                if ($PropertyValue -eq "User")
                {
                  $PropertyValue = "FALSE"
                }
                elseif ($PropertyValue -eq "System")
                {
                  $PropertyValue = "TRUE"
                }
             }
               ...
               #Checking the Testdata location mentioned in testdata CSV
                ...
                #To Call AAC Conversion Functions based on the Target Conversion Type and write the status to Testdata CSV and get the converted package path for launch and validation    
                 if($strPackage -match '.sft')
                           ...
                           WriteResultsToFile $logFile "Converting $strPackage to $strTargetType virtual format started...."
                           $ConvRetval= Validate_ASConversion $strPackage $Result $strTargetType $LogFile $ExpectedConversion_files
                           }
                  Else
                           {
...
Function Validate_ASConversion($strPackageName,$Result,$strTargetType,$logFile,$ExpectedConversion_AppVfiles)
{
$ConversionStatus=-1
 ...
 function Process_SCCMImport_CSV($csvFileLocation,$CatalogName,$PkgID,$Flag, $sessionID)
{
    ...
    If(Test-Path $csvFileLocation)
    {
        ...
        if($Flag -eq 'Import')
        {
                ...
                #Compare AS and SCCM values
                    Write-Host (Write-Header) 'sccm value':([string]$_.SCCM_VALUE).Trim()
                    Write-Host (Write-Header) 'AS value': ([string]$_.AS_VALUE).Trim()
        ...
        elseif($Flag -eq 'ReImport')
        {
             #Fetching Data from SCCM  
                ...
                #Compare AS and SCCM values
                   Write-Host (Write-Header) 'new sccm value':([string]$_.NEW_SCCM_VALUE).Trim()
                    Write-Host (Write-Header) 'new AS value': ([string]$_.NEW_AS_VALUE).Trim()
...
Catch{
        Add-Content -Path $logFile -Value "Unable to create Application in SCCM using XML:$TargetGroup"
        return -1
     }
return $result
}
...
Switch($Type)
    {
        ...
        {
            $global:UserRequirementID=Set-ASAppModelData -PackageId $SourcePackageID -Type Requirement -XmlPath $XmlPath
            $global:UserRequirementIDs= $UserRequirementID -split ","
            ValidateReturnValue $Type $UserRequirementIDs $logFile
        }
        ...
        DeviceRequirement1
        {
             $global:DeviceRequirementID1=Set-ASAppModelData -PackageId $SourcePackageID -Type Requirement -XmlPath $XmlPath
             $global:DeviceRequirementIDs1= $DeviceRequirementID1 -split ","
             ValidateReturnValue $Type $DeviceRequirementIDs1 $logFile
        }
        
        DeviceRequirement2
        {
             $global:DeviceRequirementID2=Set-ASAppModelData -PackageId $SourcePackageID -Type Requirement -XmlPath $XmlPath
             $global:DeviceRequirementIDs2= $DeviceRequirementID2 -split ","
             ValidateReturnValue $Type $DeviceRequirementIDs2 $logFile
        }

        DeviceRequirement3
        {
             $global:DeviceRequirementID3=Set-ASAppModelData -PackageId $SourcePackageID -Type Requirement -XmlPath $XmlPath
             $global:DeviceRequirementIDs3= $DeviceRequirementID3 -split ","
             ValidateReturnValue $Type $DeviceRequirementIDs3 $logFile
        }

        FreeDiskDeviceRequirement
        {
             $global:FreeDiskDeviceRequirementID=Set-ASAppModelData -PackageId $SourcePackageID -Type Requirement -XmlPath $XmlPath
             $global:FreeDiskDeviceRequirementIDs= $FreeDiskDeviceRequirementID -split ","
             ValidateReturnValue $Type $FreeDiskDeviceRequirementIDs $logFile
        }


        CoManagedDeviceRequirement
        {
           $global:CoManagedDeviceRequirementID=Set-ASAppModelData -PackageId $SourcePackageID -Type Requirement -XmlPath $XmlPath
            WriteResultsToFile $logFile "Adding CoManagedDeviceRequirement" ($CoManagedDeviceRequirementID -gt 0) $true
        }

        IntuneO365Requirement
        {
           $global:IntuneO365RequirementID=Set-ASAppModelData -PackageId $SourcePackageID -Type Requirement -XmlPath $XmlPath
            WriteResultsToFile $logFile "Adding IntuneO365Requirement" ($IntuneO365RequirementID -gt 0) $true
        }

        Dependency
        {
            $xml.SelectNodes("//Dependencies//DependencyHelper//Dependency//PackageId")[0].InnerText=$TargetPackageID
            $xml.Save($XmlPath)
	 ...
	 FileDetectionMethod
        {
            $global:FileDetectionID1=Set-ASAppModelData -PackageId $SourcePackageID -Type DetectionMethod -XmlPath $XmlPath
             $global:FileDetectionIDs1= $FileDetectionID1 -split ","
             ValidateReturnValue $Type $FileDetectionIDs1 $logFile
        }

        FolderDetectionMethod
        {
            $global:FolderDetectionID1=Set-ASAppModelData -PackageId $SourcePackageID -Type DetectionMethod -XmlPath $XmlPath
             $global:FolderDetectionIDs1= $FolderDetectionID1 -split ","
             ValidateReturnValue $Type $FolderDetectionIDs1 $logFile
        }

        RegistryDetectionMethod
        {
            $global:RegistryDetectionID1=Set-ASAppModelData -PackageId $SourcePackageID -Type DetectionMethod -XmlPath $XmlPath
             $global:RegistryDetectionIDs1= $RegistryDetectionID1 -split ","
             ValidateReturnValue $Type $RegistryDetectionIDs1 $logFile
        }

        InstallerDetectionMethod
        {
            $global:InstallerDetectionID1=Set-ASAppModelData -PackageId $SourcePackageID -Type DetectionMethod -XmlPath $XmlPath
             $global:InstallerDetectionIDs1= $RegistryDetectionID1 -split ","
             ValidateReturnValue $Type $InstallerDetectionIDs1 $logFile
        }
...
<#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  Function name   :    Function RemoveAppModelData()
  Description     :    This Function removes UserRequirement, DeviceRequirement, Dependency and Supersedence types of Appmodeldata using API's.
  Input Parameters:    $Type=Type of Appmodeldata, $SourcePackageID=Package to which Appmodeldata is added, $AppIndex= Index of app model data which is to be removed 
  Created Date    :    10/04/2020
  Last Modified   :    By: Sweta Rawat
  Last Modified   :    Date: 15/05/2020 
  Author          :    Anusha Heballi #>
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#>

Function RemoveAppModelData()
{
param(
[Parameter(Mandatory=$true)]$Type,
[Parameter(Mandatory=$true)][String]$SourcePackageID,
[Parameter(Mandatory=$true)][int]$AppIndex
)
Switch($Type)
    {
       ExeDetectionMethod
       {
         [xml] $xml = Get-ASAppModelData -PackageId $SourcePackageID -Type DetectionMethod
         $Fileid = $xml.DetectionMethods.FileDetectionMethodHelper[$AppIndex].Id
         $Folderid = $xml.DetectionMethods.FileDetectionMethodHelper[4].Id
         $Registryid = $xml.DetectionMethods.RegistryDetectionMethodHelper[$AppIndex].Id
         $Installerid = $xml.DetectionMethods.MsiDetectionMethodHelper[$AppIndex].Id
         $FileDetectionReturn = Remove-ASAppModelData -PackageId $SourcePackageID -Type DetectionMethod -Id $Fileid
         $FolderDetectionReturn = Remove-ASAppModelData -PackageId $SourcePackageID -Type DetectionMethod -Id $Folderid
         $RegisterDetectionReturn = Remove-ASAppModelData -PackageId $SourcePackageID -Type DetectionMethod -Id $Registryid
         $InstallerDetectionReturn = Remove-ASAppModelData -PackageId $SourcePackageID -Type DetectionMethod -Id $Installerid
         WriteResultsToFile $logFile "Removing file detection method" ($FileDetectionReturn -match "deleted successfully") $true
         WriteResultsToFile $logFile "Removing folder detection method" ($FolderDetectionReturn -match "deleted successfully") $true
         WriteResultsToFile $logFile "Removing registry detection method" ($RegisterDetectionReturn -match "deleted successfully") $true
         WriteResultsToFile $logFile "Removing install detection method" ($InstallerDetectionReturn -match "deleted successfully") $true        
	...
	WrapDetectionMethod
       ...
       UserRequirement
       {
         [xml] $xml = Get-ASAppModelData -PackageId $SourcePackageID -Type Requirement
         $id = $xml.Requirements.UserRequirementHelper[$AppIndex].Id
         $ReturnValue= Remove-ASAppModelData -PackageId $SourcePackageID -Type Requirement -Id $id
         WriteResultsToFile $logFile "Removing Device Requirement" ($ReturnValue -match "deleted successfully") $true
       }

       DeviceRequirement
       {
         [xml] $xml = Get-ASAppModelData -PackageId $SourcePackageID -Type Requirement
         $id = $xml.Requirements.DeviceRequirementHelper[$AppIndex].Id
         $ReturnValue= Remove-ASAppModelData -PackageId $SourcePackageID -Type Requirement -Id $id
         WriteResultsToFile $logFile "Removing Device Requirement" ($ReturnValue -match "deleted successfully") $true
       }
       ...
       {
         [xml] $xml = Get-ASAppModelData -PackageId $SourcePackageID -Type Requirement
         $id = $xml.Requirements.FreeDiskSpaceRequirementHelper[$AppIndex].Id
         $ReturnValue= Remove-ASAppModelData -PackageId $SourcePackageID -Type Requirement -Id $id
         WriteResultsToFile $logFile "Removing Device Requirement" ($ReturnValue -match "deleted successfully") $true
       }
    }
...
Switch($Type)
    {
        ...
        UserRequirement1
        {
            UpdateIds $XmlPath $UserRequirementIDs
            $URId=Set-ASAppModelData -PackageId $PackageID -Type Requirement -XmlPath $XmlPath
            $URIds= $URId -split ","
            ValidateReturnValue $Type $URIds $logFile
         }
        ...
        DeviceRequirement1
        {
            UpdateIds $XmlPath $DeviceRequirementIDs1
            $DRId1=Set-ASAppModelData -PackageId $PackageID -Type Requirement -XmlPath $XmlPath
            $DRIds1= $DRId1 -split ","
            ValidateReturnValue $Type $DRIds1 $logFile
         }

        DeviceRequirement2
        {
            UpdateIds $XmlPath $DeviceRequirementIDs2
            $DRId2=Set-ASAppModelData -PackageId $PackageID -Type Requirement -XmlPath $XmlPath
            $DRIds2= $DRId2 -split ","
            ValidateReturnValue $Type $DRIds2 $logFile
        }

        DeviceRequirement3
        {
            UpdateIds $XmlPath $DeviceRequirementIDs3
            $DRId3=Set-ASAppModelData -PackageId $PackageID -Type Requirement -XmlPath $XmlPath
            $DRIds3= $DRId3 -split ","
            ValidateReturnValue $Type $DRIds3 $logFile
        }

        FreeDiskDeviceRequirement
        {
            UpdateIds $XmlPath $FreeDiskDeviceRequirementIDs
            $FDDRId=Set-ASAppModelData -PackageId $PackageID -Type Requirement -XmlPath $XmlPath
            $FDDRIds= $FDDRId -split ","
            ValidateReturnValue $Type $FDDRIds $logFile
        }

        Dependency
        {
            $xml.SelectNodes("//DependencyHelper//Id")[0].InnerText = $DependencyID
            $xml.SelectNodes("//Dependencies//DependencyHelper//Dependency//AutoInstall")[0].InnerText =$Node1
            $xml.Save($XmlPath)
	...
	FileDetectionMethod
        {
              UpdateIds $XmlPath $FileDetectionIDs1
              $FileID = Set-ASAppModelData -PackageId $PackageID -Type DetectionMethod -XmlPath $XmlPath
              $FileIDs = $FileID -split ","
              ValidateReturnValue $Type $FileIDs $logFile
        }

        FolderDetectionMethod
        {
           UpdateIds $XmlPath $FolderDetectionIDs1
              $FolderID = Set-ASAppModelData -PackageId $PackageID -Type DetectionMethod -XmlPath $XmlPath
              $FolderIDs = $FileID -split ","
              ValidateReturnValue $Type $FolderIDs $logFile
        }

        RegistryDetectionMethod
        {
             UpdateIds $XmlPath $RegistryDetectionIDs1
              $RegistryID = Set-ASAppModelData -PackageId $PackageID -Type DetectionMethod -XmlPath $XmlPath
              $RegistryIDs = $FileID -split ","
              ValidateReturnValue $Type $RegistryIDs $logFile
        }

        InstallerDetectionMethod
        {
              UpdateIds $XmlPath $InstallerDetectionIDs1
              $InstallerID = Set-ASAppModelData -PackageId $PackageID -Type DetectionMethod -XmlPath $XmlPath
              $InstallerIDs = $FileID -split ","
              ValidateReturnValue $Type $InstallerIDs $logFile
        }
...
<#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  Function name   :    Function ValidateReturnValue($Type,$ReturnIDs,$logFile)
  Description     :    This Function checks the values returned by Set-AppModelData api, whether adding or updating any of the app model data was successfull or not
  Input Parameters:    $Type=Type of Appmodeldata, $ReturnIDs= Contains the return values after adding or updating the app model data, $logFile= Log file
...
Function ValidateReturnValue($Type,$ReturnIDs,$logFile)
{
    $return=0
    ...
    if($return -eq -1)
    {
        WriteResultsToFile $logFile "Failed adding or updating one or more $Type" $return 0
    }
    else
    {
        WriteResultsToFile $logFile "Successfully added or updated all $Type" $return 0
    }
}

<#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  Function name   :    Function UpdateIds($XmlPath,$IDs)
  Description     :    This Function Updates the IDs returned by set-appmodeldata api into the required xml so that the xml can be used to update the app model data
  Input Parameters:    $XmlPath= Xml path which will be used to update the app model data, $IDs= Values return by set-appmodeldata api after adding them
  Created Date    :    03/04/2020 
  Last Modified   :    By:------
  Last Modified   :    Date:----- 
  Author          :    Sweta Rawat #>
...
Function UpdateIds($XmlPath,$IDs)
{
    $xml=New-Object XML
    $xml.Load($XmlPath)
    $index=0
    ForEach($Id in $IDs)
    {
        $xml.SelectNodes("//Id")[$index++].InnerText= $Id
        $xml.Save($XmlPath)
    }
}
