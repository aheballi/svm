...
$TestCaseName="15IPAImport_FromSCCM_ValidateProperties"
$csvFileName="IPAImport_FromSCCM_ValidateProperties.csv"
...
$xmlpath = $TestCaseFolder + "\"+ "Facebook.xml"
$supersedes1 = $TestCaseFolder + "\"+ "BeFunky.xml"
$supersedes2 = $TestCaseFolder + "\"+ "EverNote.xml"
$supersedes3 = $TestCaseFolder + "\"+ "Flickr.xml"
$supersedes4 = $TestCaseFolder + "\"+ "InTimerEnterprise.xml"
$updatexmlpath = $TestCaseFolder + "\"+ "Facebook_Update.xml"
...
If(Select-String -Pattern "Failed" -InputObject $(Get-Content $logFile))
{
	...
	Write-host (Write-Header $TestCaseName) "IPAImport_FromSCCM_ValidateProperties Test Case Failed"
    WriteResultsToFile $logFile "IPAImport_FromSCCM_ValidateProperties Test Case Failed:" $retval -1
    $retval=-1
	exit 1
}
Else 
{
    ...
    Get-Process AdminStudioHost | Stop-Process
	Write-host (Write-Header $TestCaseName) "IPAImport_FromSCCM_ValidateProperties Test Case Passed"
    WriteResultsToFile $logFile "IPAImport_FromSCCM_ValidateProperties Test Case Passed:" $retval -1
    $retval=0
	exit 0
}
