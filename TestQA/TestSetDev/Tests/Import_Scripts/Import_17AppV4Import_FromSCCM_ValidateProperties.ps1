...
$TestCaseName="17AppV4Import_FromSCCM_ValidateProperties"
$csvFileName="AppV4Import_FromSCCM_Validate.csv"
...
$xmlpath = $TestCaseFolder + "\"+ "Hyena.xml"
$dependency1 = $TestCaseFolder + "\"+ "Adobe.xml"
$dependency2 = $TestCaseFolder + "\"+ "Firefox.xml"
$dependency3 = $TestCaseFolder + "\"+ "NestedSendToMenus.xml"
$supersedes1 = $TestCaseFolder + "\"+ "Orca.xml"
$supersedes2 = $TestCaseFolder + "\"+ "Safesign.xml"
$supersedes3 = $TestCaseFolder + "\"+ "UltraEdit.xml"  
$updatexmlpath = $TestCaseFolder + "\"+ "Hyena_Update.xml"
...
If(Select-String -Pattern "Failed" -InputObject $(Get-Content $logFile))
{
	...
	Write-host (Write-Header $TestCaseName) "AppV4Import_FromSCCM_ValidateProperties Test Case Failed"
    WriteResultsToFile $logFile "AppV4Import_FromSCCM_ValidateProperties Test Case Failed:" $retval -1
    $retval=-1
	exit 1
}
Else 
{
    ...
    Get-Process AdminStudioHost | Stop-Process
	Write-host (Write-Header $TestCaseName) "Import and reimport of APPV4 from SCCM Test Case Passed"
    WriteResultsToFile $logFile "Import and reimport of APPV4 $PackageName from SCCM Test Case Passed:" $retval -1
    $retval=0
	exit 0
}
