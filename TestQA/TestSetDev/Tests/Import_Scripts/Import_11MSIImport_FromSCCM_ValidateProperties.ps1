...
$TestCaseName="11MSIImport_FromSCCM_ValidateProperties"
$csvFileName="MSIImport_FromSCCM_Validate.csv"
...
$xmlpath = $TestCaseFolder + "\"+ "Adaware.xml"
$dependency1 = $TestCaseFolder + "\"+ "Blender.xml"
$dependency2 = $TestCaseFolder + "\"+ "Hyena.xml"
$dependency3 = $TestCaseFolder + "\"+ "GoogleEarth.xml"
$supersedes1 = $TestCaseFolder + "\"+ "Picasa.xml"
$supersedes2 = $TestCaseFolder + "\"+ "Firefox.xml"
$supersedes3 = $TestCaseFolder + "\"+ "JetAudio.xml"
$updatexmlpath = $TestCaseFolder + "\"+ "Adaware_Update.xml"
...
Write-Host "1st" $SupersedesAppId1 $SupersedesDeploymenttypeId1
...
Write-Host "2nd" $SupersedesAppId2 $SupersedesDeploymenttypeId2
...
Write-Host "3rd" $SupersedesAppId3 $SupersedesDeploymenttypeId3
...
Write-Host "4th" $DependencyAppID1 $DependencyDeploymenttypeID1
...
Write-Host "5th" $DependencyAppID2 $DependencyDeploymenttypeID2
...
Write-Host "6th" $DependencyAppID3 $DependencyDeploymenttypeID3
...
$CID= SCCM_GetCID $ModelName $SessionID 
$updatedCID=$CID[1]
Write-Host 'first id is : $updatedCID'
...
If(Select-String -Pattern "Failed" -InputObject $(Get-Content $logFile))
{
	...
	Write-host (Write-Header $TestCaseName) "MSIImport_FromSCCM_ValidateProperties Test Case Failed"
    WriteResultsToFile $logFile "MSIImport_FromSCCM_ValidateProperties Test Case Failed:" $retval -1
    $retval=-1
	exit 1
}
Else 
{
	...
	Get-Process AdminStudioHost | Stop-Process
	Write-host (Write-Header $TestCaseName) "MSIImport_FromSCCM_ValidateProperties Test Case Passed"
    WriteResultsToFile $logFile "MSIImport_FromSCCM_ValidateProperties Test Case Passed:" $retval -1
    $retval=0
	exit 0
}
