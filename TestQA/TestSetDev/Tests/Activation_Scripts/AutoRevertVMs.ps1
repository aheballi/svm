...
$AdminStudioVersion="Ising R2"
$Global:ScriptName="Revert VMs and Install Latest $AdminStudioVersion Build and Run Activation Tests"
...
if($LastBuildNo -ne $global:GreatestBuildNo )
  {
    ...
    if($BuildRetVal -eq 0)
    ...
    else
      {
        ...
        Write-Message ("Reverting VMs not Necessary..as the Lastest Build is not present...") $LogFile
        Write-Message ("Copying Latest $AdminStudioVersion Build to Shared Location: Failed") $DetLogFile
        Write-Message ("Reverting VMs not Necessary..as the Lastest Build is not present...") $DetLogFile
       }
