...
{
	TCHAR sSerial[_MAX_PATH] = {0};
	DWORD dwCount = _MAX_PATH;

	if (ERROR_SUCCESS == MsiGetProperty(hMsi, _T("PIDKEY"),sSerial,&dwCount))
	{
		HKEY hKey;
		...
		{
			TCHAR sInstallDir[_MAX_PATH] = {0};
			dwCount = _MAX_PATH;
			if (ERROR_SUCCESS == MsiGetProperty(hMsi, _T("INSTALLDIR"),sInstallDir,&dwCount))
			{
				LPTSTR lpPtr = sInstallDir;
				if (lpPtr)
				{
					while(*lpPtr)
						lpPtr++;
					...
					wsprintf(sFile,_T("\"%s\\Support\\WebReg.exe\" %s"),sInstallDir, sSerial);
