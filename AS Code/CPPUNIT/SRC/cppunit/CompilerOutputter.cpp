...
CompilerOutputter::printSucess()
{
  m_stream  << "OK (" << m_result->runTests()  << ")"  
            <<  std::endl;
/*
  int nRunTests = m_result->runTests();
  for (int nIndex = 0; nIndex < nRunTests; ++nIndex)
  {
	  const TestResultCollector::Tests &oTests = m_result->tests();
	  Test *pTest = oTests[nIndex];
	  m_stream  << "\t (" << nIndex + 1 << ") "  << pTest->toString()
				<<  std::endl;
  }
*/
}
